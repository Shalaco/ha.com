 ...
 * 
 * This function takes a SlideDeck object and
 * tests the structure to determine if it has vertical slides.
 * This is used to tell the SlideDeckVideoAPIs class and listeners
 * that the traversal and indexing should be adjusted. 
 */
...
function __slidedeck2_isMobile(){
    ...
    if(
        ...
        navigator.userAgent.match(/like Mac OS X/i)
        // If iOS or Android
        || (navigator.userAgent.match(/android/i) || navigator.userAgent.match(/like Mac OS X/i))
        // If iPad - Specifically
        || navigator.userAgent.match(/iPad/i)
    ) mobile = true;
...
var SlideDeckOverlay = function(elem){
    ...
    // Speed of individual animations
    this.speed = 200;
...
// Extensible Overlay processing object, key all functions by their type
var SlideDeckOverlays = {
    // Click event actions
    actions: {},
    // Initialization and setup actions
    inits: {}
};
...
(function($){
    ...
    window.SlideDeckVideoAPIs = function( slidedeck ){
        ...
        if (navigator.userAgent.match(/like Mac OS X/i)) {
            // Auto Starting of the next video is buggy on iOS.
            autoStartNextVideo = false;
        }
        // If iOS or Android
        if (navigator.userAgent.match(/android/i) || navigator.userAgent.match(/like Mac OS X/i)) {
            // Auto resuming the playback on slide navigation:
            // this can be troublesome due to the native "pop-out" player.
            autoResumePlayback = false;
        }
        // If iPad - Specifically
        if (navigator.userAgent.match(/iPad/i)) {
            // This is OK on the iPad as there's no "pop-out" player.
         ...
         * 
         * Checks to see if the video is indeed paused 
         * (using a few different methods) and if it in indeed
         * paused, it will resume playback.
         */
            ...
            switch( playerType ){
                case 'youtube':
                    if( typeof( player ) != 'undefined' ){
                        // So... this is only something that we want to do if the video isn't ended or
                        // if the video isn't started. Vimeo (below) has a method to unload. YouTube does not.
                ...
                case 'vimeo':
                    if( typeof( player ) != 'undefined' ){
                        // It seems that the API functions that return a value need to do so via a
                        // callback function... alright... I'll play along.
                ...
                case 'dailymotion':
                    if( typeof( player ) != 'undefined' ){
                        // For the dailymotion case, we'll subtract a second from the video's end.
                        // This is because the video can sometimes end just before the duration time defined.
        ...
        function playVideo( index, playerType, deckId ){
            ...
            if( typeof( player ) != 'undefined' ){
                ...
                // Add a video playing class to the deck
        ...
        /**
         * Here's the function that we call when a playing video ends.
         * 
         * It's called by all three (at the time of this writing) APIs and 
         * will offer to advance the slide and potentially play the next video.
         */
        this.videoEnded = function( index, playerType, deckId ){
            ...
            // The above Hmmmms are because I can't find a reliable way to unload/reset the video's
            // playback state for YouTube and Dailymotion.
            
            if( advanceAtVideoEnd ){
                // If the deck is vertical, we need to make a few adjustments.
                var deckContext = deckElement.slidedeck();
                if( __isVerticalDeck( deckElement ) )
                    deckContext = deckContext.vertical();
                
                // Move to the next slide if the option is true.
                deckContext.next(function(){
                    var current = deckContext.current;
                    var currentSlide = $( '#' + deckId ).find('dd:eq(' + ( current - 1 ) + ')');
                    
                    // More vertical accommodations.
                    if( __isVerticalDeck( deckElement ) ){
                        current += 1;
                    ...
                    // Start playing the video on the destination slide if the
                    // option is true. Note: iOS and Android produce bad UX when 
                    // this option is set to true. It's being overriden above.
        ...
        // Pauses all the active players in the deck,
        // switches by video player type.
        ...
        this.slideDeckPauseAllVideos = function(thedeck) {
            ...
            for( var k in players ){
                ...
                // Remove the video playing class from the deck
                slidedeckFrame.removeClass('sd2-video-playing');
                
                // Different commands for different videos.
                switch( player.playerType ){
                    case 'youtube':
                        if( typeof( player.getPlayerState ) == 'function' ){
                            // For YouTube, we'll check the player state before pausing.
                            // This is important because we depend on the player state elsewhere;
                            // we don't want to erroneously set the state to paused.
                    ...
                    break;
                    case 'vimeo':
                        // We removed a conditional here that seemed to be superfluous.
                        player.api('pause');
                    break;
                    case 'dailymotion':
                        // Here we're checking the paused property even though
    ...
    SlideDeckFadingNav.prototype.checkVertical = function(slidedeck){
        // When being run as a before() action
        ...
        // When being called independently from the prototype, use the passed in SlideDeck object
        else {
        ...
        // Do nothing if no vertical slides
        if(frame.hasClass('no-vertical-slide')){
            return false;
        }
        
        // Start with everything showed by default
        prevButton.show();
        nextButton.show();
    
        // Don't hide anything if cycle is turned on
        ...
        if(typeof(slidedeck.vertical()) != 'undefined'){
            ...
            // First slide, hide previous button
    ...
    // SlideDeckOverlay Class initialization
    SlideDeckFadingNav.prototype.initialize = function(elem){
        ...
        // Fail silently for those SlideDecks that have no navigation
        ...
        // Adding global prev/next bindings
        ...
        // Hide the horizontal navigation if we don't have more than one slide
    ...
    SlideDeckOverlay.prototype.initialize = function(elem){
        ...
        // The SlideDeck DOM element
        this.elems.slidedeck = $(elem);
        // The SlideDeck frame element
        this.elems.frame = this.elems.slidedeck.closest('.slidedeck-frame');
        
        // Fail silently if this is a malformed SlideDeck for overlays
        if(this.elems.frame.length < 1){
            return false;
        }
        
        // Fail silently if overlays don't exist for manual decks
        ...
        this.elems.container.delegate('.' + this.classes.overlay, 'click', function(event){
            // Get $(this) from the data cache
            var $this = $.data(this, '$this');
            
            // If $(this) is not cached, get it and cache it
            if(!$this){
                $this = $(this);
                // Store $(this) in the data cache
                $.data(this, '$this', $this);
            }
            ...
            // Run the Overlay type's action if it exists
        ...
        this.elems.overlays.each(function(ind){
            ...
            // Run the Overlay type's init function if it exists
    ...
    SlideDeckOverlay.prototype.toggle = function(){
        if(!this.elems.container.hasClass('open'))
            this.open();
        else
            this.close();
    };
    
    // Facebook Overlay link click action
    SlideDeckOverlays.actions['facebook'] = function(elem, event){
        ...
        // Get the parent window (which can also be the current window) in case its an iframe SlideDeck embed
        var $window = $(parent);
        
        // Setup offsets for positioning
        var offset = {
            top: parent.screenY || parent.screenTop,        // screenTop for IE compatibility
            left: parent.screenX || parent.screenLeft,      // screenLeft for IE compatibility
            width: $window.outerWidth(),
            height: $window.outerHeight()
        };
        
        // Get popup dimensions from data attributes from the PHP object
        var pos = {
 ...
 * @package SlideDeck
 * @subpackage SlideDeck 2.0
 * @version 1.0
 */
var SlideDeckCoverPostProcessFront = {};
var SlideDeckCoverPostProcessBack = {};
var SlideDeckCover = function(elem){
    // Elements used by this Cover
    this.elems = {};
    
    // Original slidedeckOptions
    this.slidedeckOptions = {};
    
    // Does this SlideDeck have a Front Cover?
    this.hasFront = false;
    // Does this SlideDeck have a Back Cover?
    this.hasBack = false;
    
    // Classes used by this Cover
    this.classes = {
    ...
    // Visible status of the back cover
    this.backCoverVisible = false;
    
    // Style of the cover
    this.coverStyle = "";
    
    // Initialize the Cover for this SlideDeck
    this.initialize(elem);
};
...
(function($){
    ...
    SlideDeckCover.prototype.initialize = function(elem){
        ...
        if( coverMatches ){
            ...
            // Check if we have a front cover and initialize it if we do
    ...
    SlideDeckCover.prototype.initializeBack = function(){
        ...
        // SlideDeck back cover wrapper element
        ...
        this.slidedeck.setOption('before', function(slidedeck){
            ...
            if(
                ((slidedeck.current == 1 && slidedeck.former == slidedeck.slides.length)     // Cycle boolean(true)
                || (slidedeck.current == slidedeck.former && slidedeck.current == slidedeck.slides.length && slidedeck.options.cycle == false))   // Cycle boolean(false)
        ...
        // Run the CTA PostProcess function for the hover color
        if( typeof( SlideDeckCoverPostProcessCTA ) == 'function' ){
            SlideDeckCoverPostProcessCTA( this.elems.restart );
        }
        
        if(SlideDeckCoverPostProcessBack[this.coverStyle])
            SlideDeckCoverPostProcessBack[this.coverStyle](this.elems.restart, this.elems.frame.hasClass('slidedeck-cover-peek'));
    ...
    SlideDeckCover.prototype.initializeFront = function(){
        ...
        // SlideDeck front cover wrapper element
        ...
        // SlideDeck cover open button
        this.elems.open = this.elems.frontCover.find('.' + this.classes.open);
        
        // Hide SlideDeck navigation
        this.elems.frame.addClass('force-nav-hidden');
        this.elems.overlay.css('opacity', 0);
        
        // Pause autoplay to prevent automatic progression while the cover is closed
        this.slidedeck.pauseAutoPlay = true;
        this.slidedeck.setOption('keys', false);
        this.slidedeck.setOption('scroll', false);
        ...
        if(SlideDeckCoverPostProcessFront[this.coverStyle])
            SlideDeckCoverPostProcessFront[this.coverStyle](this.elems.open, this.elems.frame.hasClass('slidedeck-cover-peek'));
    ...
    var SlideDeckCoverPostProcessCTA = function( $button ){
        ...
        $button.parent().delegate('.slidedeck-cover-cta', 'mouseenter mouseleave', function(event){
            if( event.type == 'mouseenter' ){
                $(this).find('.slidedeck-cover-color').css({ backgroundColor: hoverColor });
            }else{
                $(this).find('.slidedeck-cover-color').css({ backgroundColor: accentColor });
            }
        });
    };
    ...
    $(document).ready(function(){
        $('.slidedeck').each(function(){
            if(!$.data(this, 'SlideDeckCover'))
                $.data(this, 'SlideDeckCover', new SlideDeckCover(this));
        });
    });
})(jQuery);
